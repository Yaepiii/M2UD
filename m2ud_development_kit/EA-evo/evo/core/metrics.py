...
class PE(Metric):
    ...
    def get_statistic(self, statistics_type: StatisticsType) -> float:
        ...
        elif statistics_type == StatisticsType.drift:
            return self.drift
        else:
...
class APE(PE):
    ...
    def process_data(self, data: PathPair) -> None:
        ...
        if self.pose_relation in (PoseRelation.translation_part,
            ...
            raise MetricsException("unsupported pose_relation")
        squared_errors = np.power(self.error, 2)
        error_rmse =  math.sqrt(np.sum(squared_errors))
        self.drift = error_rmse / traj_est.path_length * 100
    
    def process_data_with_efficiency(self, data: PathPair, time_consumption_path: str = "", sensor_frequency: float = 10) -> None:
        """
        Calculates the APE on a batch of SE(3) poses from trajectories.
        :param data: tuple (traj_ref, traj_est) with:
        traj_ref: reference evo.trajectory.PosePath or derived
        ...
        # obtain time consumption data
        time_consumptions = []
        with open(time_consumption_path, 'r') as file:
            for line in file:
                timestamp, duration = line.strip().split()
                time_consumptions.append([float(timestamp), float(duration)])
        # time alignment
        efficiency_list = []
        ref_timestamps = traj_ref.timestamps
        for i in range(len(ref_timestamps)):
            ref_timestamp = ref_timestamps[i]
            for j in range(len(time_consumptions)):
                time_consumption = time_consumptions[j]
                if time_consumption[0] - 0.15 < ref_timestamp < time_consumption[0] + 0.15:
                    efficiency_list.append(time_consumption[1])
                    break
                
        if len(efficiency_list) != traj_est.num_poses:
            raise MetricsException(
                "time consumption must have same number of poses")

        efficiency = np.array(efficiency_list)
        frame_time = 1000 / sensor_frequency   # ms
        efficiency = (efficiency - frame_time) / frame_time
        ...
        if self.pose_relation in (PoseRelation.translation_part,
            ...
            # E is an array of position vectors only in this case
            self.error = np.array([np.linalg.norm(E_i) for E_i in self.E])
            self.error = efficiency * self.error
        elif self.pose_relation == PoseRelation.rotation_part:
            self.error = np.array([
                np.linalg.norm(lie.so3_from_se3(E_i) - np.eye(3))
                for E_i in self.E
